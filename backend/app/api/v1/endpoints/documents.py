...
router = APIRouter()
crud_document = TenantCRUDBase(Document)
...
async def list_documents(
    ...
    return await crud_document.get_multi(session, tenant_id, skip=skip, limit=limit, filters=filters)
...
async def create_document(
    ...
    return await crud_document.create(session, obj_in=document, tenant_id=tenant_id)
...
async def get_document(
    ...
    return await crud_document.get_or_404(session, document_id, tenant_id)
...
async def update_document(
    ...
    """Update a document."""
    db_document = await crud_document.get_or_404(session, document_id, tenant_id)
    ...
    return await crud_document.update(session, db_obj=db_document, obj_in=document_update, tenant_id=tenant_id)
...
async def delete_document(
    ...
    deleted = await crud_document.delete(session, id=document_id, tenant_id=tenant_id)
    if not deleted:
...
async def create_new_version(
    ...
    return await crud_document.create(session, obj_in=new_document, tenant_id=tenant_id)
